                ...
                try {
                    if (typeof callback === 'function') {
                        this.adapter.getForeignObjects('*', 'state', 'rooms', {user: socket._acl.user}, async (error, objs) => {
                            try {
                                const channels = await this.adapter.getForeignObjects('*', 'channel', null, {user: socket._acl.user});
                                const devices = await this.adapter.getForeignObjects('*', 'device', null, {user: socket._acl.user});
                                const enums = await this.adapter.getForeignObjects('*', 'enum', null, {user: socket._acl.user});
                                const config = await this.adapter.getForeignObjectAsync('system.config', {user: socket._acl.user});
                                Object.assing(objs, channels, devices, enums);
