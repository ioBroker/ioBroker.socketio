...
function IOBrokerSocket(server, settings, adapter) {
            ...
            if (!socket._sessionTimer) {
                socket._sessionTimer = setTimeout(function () {
                    ...
                    adapter.getSession(socket._sessionID, function (obj) {
                        if (obj) {
                            adapter.setSession(socket._sessionID, settings.ttl, obj);
                        } else {
                            socket.emit('reauthenticate');
                        }
                    });
    ...
    function socketEvents(socket, user) {
        ...
        if (socket.handshake.headers.cookie) {
            ...
            if (m) {
                ...
                if (sessionID) {
                    ...
                    that.adapter.getSession(sessionID, function (obj) {
                        if (obj && obj.passport) {
        ...
        socket.on('authenticate', function (user, pass, callback) {
            console.log('Request authenticate [' + socket._user + ']');
        ...
        socket.on('subscribeObjects', function (pattern) {
            updateSession(socket);
            subscribe(this, 'objectChange', pattern)
        });

        socket.on('unsubscribeObjects', function (pattern) {
            updateSession(socket);
