...
function IOBrokerSocket(server, settings, adapter) {
    ...
    var __construct = (function () {
        ...
        // enable permissions checking if run as submodule
        if (that.settings.permissions) that.settings.auth = true;
    ...
    function calculatePermissions(socket, user, callback) {
        ...
        that.adapter.getForeignObjects('*', 'group', function (err, groups) {
    ...
    function socketEvents(socket, user) {
        ...
        console.log((new Date()).toISOString() + ' Connected ' + user);
        socket._user = user || that.adapter.config.defaultUser;
            ...
            if (m) {
                ...
                if (sessionID) {
                    ...
                    that.settings.store.get(sessionID, function (err, obj) {
                        ...
                        console.log('Got sessionID for ' + socket._user);
                        if (socket._authPending) {
                            socket._authPending(!!socket._user, true);
                            delete socket._authPending;
                        }
                    ...
                    socket._user = that.adapter.config.defaultUser;
                }
            } else {
                socket._user = that.adapter.config.defaultUser;
            }
        ...
        socket.on('authenticate', function (user, pass, callback) {
            console.log((new Date()).toISOString() + ' Request authenticate [' + socket._user + ']');
            if (typeof user == 'function') {
                callback = user;
                user = undefined;
            }
            if (socket._user !== null) {
                callback(socket._user !== null, socket._secure);
            } else {
        ...
        socket.on('authEnabled', function (callback) {
            if (updateSession(socket) && checkPermissions(socket, 'authEnabled', callback)) {
                callback(that.adapter.config.auth, socket._user);
            }
        });
        ...
        socket.on('readFile', function (_adapter, fileName, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'readFile', callback, fileName)) {
                that.adapter.readFile(_adapter, fileName, {user: 'system.user.' + (socket._user || that.adapter.config.defaultUser)}, callback);
            }
        });
        ...
        socket.on('readFile64', function (_adapter, fileName, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'readFile64', callback, fileName)) {
                that.adapter.readFile(_adapter, fileName, {user: 'system.user.' + (socket._user || that.adapter.config.defaultUser)}, function (err, buffer) {
                    var data64;
                    if (buffer) {
        ...
        socket.on('writeFile64', function (_adapter, fileName, data64, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'writeFile64', callback, fileName)) {
                ...
                that.adapter.writeFile(_adapter, fileName, buffer, {user: 'system.user.' + (socket._user || that.adapter.config.defaultUser)}, function (err) {
                    if (callback) {
                        callback(err);
                    }
                });
        ...
        socket.on('writeFile', function (_adapter, fileName, data, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'writeFile', callback, fileName)) {
                that.adapter.writeFile(_adapter, fileName, data, {user: 'system.user.' + (socket._user || that.adapter.config.defaultUser)}, callback);
            }
        });

        socket.on('unlink', function (_adapter, name, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'unlink', callback, name)) {
                that.adapter.unlink(_adapter, name, {user: 'system.user.' + (socket._user || that.adapter.config.defaultUser)}, callback);
            }
        });
        ...
        socket.on('rename', function (_adapter, oldName, newName, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'rename', callback, oldName)) {
                that.adapter.rename(_adapter, oldName, newName, {user: 'system.user.' + (socket._user || that.adapter.config.defaultUser)}, callback);
            }
        });

        socket.on('mkdir', function (_adapter, dirName, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'mkdir', callback, dirName)) {
                that.adapter.mkdir(_adapter, dirName, {user: 'system.user.' + (socket._user || that.adapter.config.defaultUser)}, callback);
            }
        });
        ...
        socket.on('readDir', function (_adapter, dirName, callback) {
            if (updateSession(socket) && checkPermissions(socket, 'readDir', callback, dirName)) {
                that.adapter.readDir(_adapter, dirName, {user: 'system.user.' + (socket._user || that.adapter.config.defaultUser)}, callback);
            }
        });
