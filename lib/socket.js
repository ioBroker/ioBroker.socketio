   ...
   // Extract user name from socket
    ...
    this.unsubscribe = function (socket, type, pattern) {
        ...
        if (socket) {
            ...
            for (var pattern in this.subscribes[type]) {
    ...
    function socketEvents(socket) {
        ...
        /*if (that.settings.apikey) {
            socket.emit('apikey', that.settings.apikey, that.settings.version, that.settings.uuid, function (err) {
                if (!err) {
        ...
        if (!socket.conn) {
            ...
            socket.on('cloudConnect', function () {
                // The cloud client must send all subscribes anew. So no automatical subscribe
