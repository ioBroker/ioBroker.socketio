   ...
   // Extract user name from socket
        ...
        if (this.subscribes[type][pattern] === undefined) {
            ...
            if (type === 'stateChange') {
                ...
                if (this.adapter.subscribeForeignObjects) {
                    this.dapter.subscribeForeignObjects(pattern);
                }
            } else if (type === 'log') {
    ...
    function showSubscribes(socket, type) {
        if (socket && socket._subscribe) {
            ...
            that.adapter.log.debug('Subscribes: no subcribes');
        }
    }
    ...
    function addGroup(group, desc, acl, options, callback) {
        ...
        that.adapter.getForeignObject('system.group.' + group, options, function (err, obj) {
            if (obj) {
                ...
                that.adapter.setForeignObject('system.group.' + group, {
                    type: 'group',
                    common: {
                        name: name,
                        desc: desc,
    ...
    function socketEvents(socket) {
        ...
        if (that.settings.clientid) {
            socket.emit('apikey', that.settings.clientid, that.settings.version, that.settings.uuid);
        }
