...
class SocketCommon {
    ...
    _unsubscribeAll() {
        if (this.server && this.server.ioBroker) {
            ...
            Object.keys(this.server.sockets.sockets).forEach(i =>
                this.commands.unsubscribeSocket(this.server.sockets.sockets[i]));
        } else
            ...
            this.infoTimeout = setTimeout(() => {
                ...
                if (this.server) {
                    ...
                    if (this.server.sockets) {
                        // this could be an object or array
                        Object.keys(this.server.sockets.sockets).forEach(i =>
                            clientsArray.push(this.server.sockets.sockets[i]._name || 'noname'));
                    }
    ...
    close() {
        ...
        if (this.server && this.server.sockets) {
            // this could be an object or array
            Object.keys(this.server.sockets.sockets).forEach(i => {
                const socket = this.server.sockets.sockets[i];
                if (socket._sessionTimer) {
                    clearTimeout(socket._sessionTimer);
                    socket._sessionTimer = null;
                }
